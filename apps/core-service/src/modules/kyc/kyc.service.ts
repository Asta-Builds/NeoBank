...
export class KycService {
  ...
  async createCase(userId: string): Promise<KycCase> {
    const existingCase = await this.kycRepository.findOne({
      where: { userId },
    });
    if (existingCase) {
    ...
    this.logger.log(
      `Created KYC case ${savedCase.id} for user ${userId}. URL: ${providerResult.verificationUrl}`,
    );
  ...
  async updateStatus(caseId: string, status: KycStatus): Promise<KycCase> {
    const kycCase = await this.kycRepository.findOne({ where: { id: caseId } });
    if (!kycCase) {
      throw new NotFoundException('KYC case not found');
    }

    kycCase.status = status;
    if (status === KycStatus.APPROVED || status === KycStatus.REJECTED) {
      kycCase.resolvedAt = new Date();
    }

    return await this.kycRepository.save(kycCase);
  }
}
