"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var KycService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.KycService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const kyc_case_entity_1 = require("./entities/kyc-case.entity");
const kyc_status_enum_1 = require("./enums/kyc-status.enum");
const wallets_service_1 = require("../wallets/wallets.service");
const wallet_status_enum_1 = require("../wallets/enums/wallet-status.enum");
const idenfy_provider_1 = require("./providers/idenfy.provider");
let KycService = KycService_1 = class KycService {
    kycRepository;
    walletsService;
    idenfyProvider;
    logger = new common_1.Logger(KycService_1.name);
    constructor(kycRepository, walletsService, idenfyProvider) {
        this.kycRepository = kycRepository;
        this.walletsService = walletsService;
        this.idenfyProvider = idenfyProvider;
    }
    async createCase(userId) {
        const existingCase = await this.kycRepository.findOne({ where: { userId } });
        if (existingCase) {
            throw new common_1.ConflictException('KYC case already exists for this user');
        }
        const providerResult = await this.idenfyProvider.createSession(userId);
        const kycCase = this.kycRepository.create({
            userId,
            providerCaseId: providerResult.providerCaseId,
            status: providerResult.status,
        });
        const savedCase = await this.kycRepository.save(kycCase);
        this.logger.log(`Created KYC case ${savedCase.id} for user ${userId}. URL: ${providerResult.verificationUrl}`);
        return savedCase;
    }
    async getCaseByUserId(userId) {
        const kycCase = await this.kycRepository.findOne({ where: { userId } });
        if (!kycCase) {
            throw new common_1.NotFoundException('KYC case not found');
        }
        return kycCase;
    }
    async approveCase(caseId) {
        const kycCase = await this.kycRepository.findOne({ where: { id: caseId } });
        if (!kycCase) {
            throw new common_1.NotFoundException('KYC case not found');
        }
        if (kycCase.status === kyc_status_enum_1.KycStatus.APPROVED) {
            return kycCase;
        }
        kycCase.status = kyc_status_enum_1.KycStatus.APPROVED;
        kycCase.resolvedAt = new Date();
        const savedCase = await this.kycRepository.save(kycCase);
        const wallet = await this.walletsService.createWallet(kycCase.userId);
        await this.walletsService.updateStatus(wallet.id, wallet_status_enum_1.WalletStatus.ACTIVE);
        return savedCase;
    }
};
exports.KycService = KycService;
exports.KycService = KycService = KycService_1 = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(kyc_case_entity_1.KycCase)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        wallets_service_1.WalletsService,
        idenfy_provider_1.IdenfyProvider])
], KycService);
//# sourceMappingURL=kyc.service.js.map